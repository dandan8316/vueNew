<template>
    <div id="myMessage">
        <div class="myMessage">
            <!-- 顶部的导航栏 -->
            <van-nav-bar :title="$t('m.taolumi')" left-arrow @click-left="onClickLeft" fixed/>
            <ul class="customer_service">
                <li class="customerBox" v-for="(item, index) in dataLists" :key="item.id">
                    <div class="leftIcon">
    ...
    export default {
        data() {
            return {
                ...
                // 是否有消息
                isshownomessage: true
            }
        },
        created() {
            this.msg.msgCategory = this.$route.query.msgCategory
            let msgList = qs.stringify(this.msg);
            this.$axios.post(this.api.globalApi.systemMyMsgList, msgList)
                .then(res => {
                    //console.log(res)
